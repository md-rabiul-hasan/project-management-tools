...
export default function Revised({deleteTask, handleEditTask}) {
    const { projectData } = useContext(ProjectContext);
    const baseData = projectData.filter(project => project.category === 'revised');
    const [lowToHighSort, setLowToHighSort] = useState(false);

    const handleSorting = () => {
        setLowToHighSort(prevState => !prevState);
    };
        ...
        <div className="mb-4 w-full px-2 sm:w-1/2 md:w-1/4">
            <div className="rounded-lg bg-rose-500 p-4">
                <div className="mb-2 flex items-center justify-between">
                    <h3 className="text-lg font-semibold">Revised ({filteredProjectData.length})</h3>
                    <svg
